...
function CheckInventory(props) {
  ...
  if ( isInventory ) {
    return <MainPageInvenGrid isInventory={ isInventory } items={ myItems } setMyItem={ props.setMyItem } setGoldStatus={ props.setGoldStatus } />
  }
  else {
    // inven grid 형태로 넘겨준다. isInventory를 넘겨주는 것은, 컴포넌트를 최대한 활용하기위함
    return <MainPageInvenGrid isInventory={ isInventory } items={ storeItems } setMyItem={ props.setMyItem } setGoldStatus={ props.setGoldStatus }/>
  }

}

export default function Inventory({ myItem, setMyItem, setGoldStatus }) {
  ...
  return (
    <View style={styles.containerUnder}>
      ...
      <CheckInventory isInventory={ isInventory } myItem={ myItem } setMyItem={ setMyItem } setGoldStatus={ setGoldStatus } />
